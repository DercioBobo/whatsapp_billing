...
"""
WhatsApp Message Usage Reconciliation Report
=============================================
Compares every WhatsApp Message Usage Log against the CURRENT state of the
Sales Invoice it claims to belong to, and flags where they've drifted apart.

Drift happens because a Sales Invoice only tracks one active billing month
(``wmb_billing_month``) and one billing line per Item. If usage for a second,
different month is fetched onto an invoice that already carries messages for
an earlier month, the earlier month's Usage Log is left pointing at an
invoice whose line no longer reflects it. This report surfaces exactly that
so it can be corrected by hand — either by editing the Usage Log (re-link
``sales_invoice``, adjust ``total_messages`` / ``price_per_message``) or by
fixing the invoice directly while it's still a draft.

Columns
-------
Customer | Log Month | Log Messages | Log Amount | Log Status |
...
def _get_columns():
    return [
        ...
        {"fieldname": "total_messages", "label": _("Log Messages"), "fieldtype": "Int", "width": 100},
...
def _get_data(filters):
    ...
    rows = frappe.db.sql(
        ...
        FROM
            ...
            LEFT JOIN `tabWhatsApp Message Billing Config` wmc
                   ON wmc.customer = wl.customer AND wmc.is_active = 1
            LEFT JOIN `tabSales Invoice Item` sii
                   ON sii.parent = si.name AND sii.item_code = wmc.billing_item
        {where}
        ORDER BY
...
def _match_status(row):
    ...
    if int(row.invoice_item_qty) != int(row.total_messages or 0):
